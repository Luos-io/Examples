...
void LcdDrv_Print(char* text, int length)
{
    ...
    for (int i=0; i<length; i++)
    {
        ...
        if (FOURBITMODE) LcdDrv_Write(letter>>4);    
...
void LcdDrv_SetParameter(uint8_t param, uint8_t value)
{
    ...
    {
    case LCD_DISPLAY:
        if (value)      display_control |= LCD_DISPLAYON;
        else            display_control &= ~LCD_DISPLAYON;
        
        LcdDrv_SendCommand(LCD_DISPLAYCONTROL | display_control);
        break;

    case LCD_CURSOR:
        if (value)      display_control |= LCD_CURSORON;
        else            display_control &= ~LCD_CURSORON;

        LcdDrv_SendCommand(LCD_DISPLAYCONTROL | display_control);
        break;

    case LCD_BLINK:
        if (value)      display_control |= LCD_BLINKON;
        else            display_control &= ~LCD_BLINKON;

        LcdDrv_SendCommand(LCD_DISPLAYCONTROL | display_control);
        break;

    case LCD_AUTOSCROLL:
        if (value)      display_mode |= LCD_ENTRYSHIFTINCREMENT;
        else            display_mode &= ~LCD_ENTRYSHIFTINCREMENT;
    ...
    case LCD_RIGHT_TO_LEFT:
        if (value) display_mode &= ~LCD_ENTRYLEFT;
        else display_mode |= LCD_ENTRYLEFT;
...
void LcdDrv_Write(uint8_t value)
{
    if (RW_Pin != 255) HAL_GPIO_WritePin(RW_GPIO_Port, RW_Pin, GPIO_PIN_RESET);

    for (int i = 0; i < (8 - 4*FOURBITMODE); i++)
    {
    	if(value>>i&0x01)	HAL_GPIO_WritePin(data_gpio_port[i], data_pins[i], GPIO_PIN_SET);
    	else                HAL_GPIO_WritePin(data_gpio_port[i], data_pins[i], GPIO_PIN_RESET);
    }
...
{
    HAL_GPIO_WritePin(RS_GPIO_Port, RS_Pin, GPIO_PIN_RESET);
    if (FOURBITMODE) LcdDrv_Write(value>>4);
    LcdDrv_Write(value);
}
