...
uint8_t FingerprintDrv_Enroll(void)
{
  ...
  if (Fingerprint_Image2Tz(1) != FINGERPRINT_OK) return false;
  ...
  if (Fingerprint_Image2Tz(2) != FINGERPRINT_OK) return false;

  if (Fingerprint_CreateModel() != FINGERPRINT_OK) return false;

  if (Fingerprint_StoreModel(Fingerprint_GetTemplateCount()+1) != FINGERPRINT_OK) return false;
...
uint8_t FingerprintDrv_DeleteAll(void)
{
  ...
  {
    if (Fingerprint_EmptyDatabase() == FINGERPRINT_OK) return true;
  }
...
{
  if (Fingerprint_GetTemplateCount()==0) return true;
  ...
  if (Fingerprint_SearchFinger() != FINGERPRINT_OK) return false;
